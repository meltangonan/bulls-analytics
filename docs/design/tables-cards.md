...
| Game Score band color | `house.game_score_fill` (fixed explainer bands: 20+ very good, 30+ star-level, 40+ dominant, 50+ historic) | [Height ladder](../../scripts/prototypes/game_score_by_height.py), rookie/sophomore boxed cards; [season openers](../../scripts/prototypes/season_opener_performances.py) passes it to `render_chart(score_fill=...)` in place of the red card. Skip it when every row lands in one band (Taj Gibson retirement) |
