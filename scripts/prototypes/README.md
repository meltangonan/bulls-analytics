...
| `assist_duos.py` + `assist_duos_fetch.py` | Bulls assist duos — which pairs created the most baskets for one another. `assist_duos_fetch.py` caches every assisted Bulls basket since 2000–01 from `PlayByPlayV3` (one CSV per season, strictly serial: NBA.com throttles concurrent play-by-play hard) and `--reconcile` checks each season against official box-score assists — all 26 currently match exactly. `assist_duos.py --mode both` renders two visual projects: `assist-duos` (best connection of every season, newest first, one slide per decade, plus a 2025–26 top-eight slide) and `assist-duos-by-decade` (top ten within each decade). Bar length is combined assists and the colour break is the split between directions; second column is games played together, since shared minutes do not exist before 2007–08 (needs network only when a season cache is missing) |
