...
| `derrick_rose_bulls_zone_charts.py` | Derrick Rose's seven played Bulls regular seasons plus an attempt-weighted Chicago-tenure total — excludes the no-appearance 2012-13 season, reconciles every included season to official Bulls FGA and FGM, compares each twelve-zone result with the matching season's NBA baseline, uses the universal 20-FGA player-season colour floor and a seven-season 140-FGA tenure floor, and records unlocated historical league attempts rather than assigning them to invented zones. |
