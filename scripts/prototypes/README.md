...
| `taj_gibson_retirement_data.py` | Taj Gibson's Bulls career in six tables: per-game season splits (regular and playoff, from `PlayerCareerStats` for games started), best five by Game Score, career highs with tie counts, and twelve franchise ranks; reconciles `FranchisePlayers` against the cached `top_game_performances` logs and records each rank's pool, because blocks, steals and rebound splits predate NBA tracking | `tests/test_taj_gibson_retirement.py` |
| `taj_gibson_bulls_zone_charts.py` | Fifth in the tenure zone-chart family (after Rose, DeRozan, Butler, Hinrich, Buzelis): renders only the pooled Bulls-tenure page plus a career shot diet from `bulls.analysis.shot_families`. Reconciles each season's shots to official FGA and drops the handful of unlabelled league rows older seasons carry, which post-2024 seasons do not | `tests/test_taj_gibson_retirement.py` |
| `taj_gibson_retirement_tables.py` | Renders those CSVs as plain Basketball-Reference-style assets; `--slide` picks one. `top-games-table` is the accent-card table of the 2025-26 best-games post, reusing `house.draw_accent_card`, minus the three-point and free-throw splits; the playoff round/game marker is derived from the game id rather than hand-kept. Rejected treatments (rank dot plot, boxed-card leaderboard, career-high cards, banded ranks, pie and 100% bar shot diets) were removed once chosen against; their renders stay in the project's asset history. Competing treatments also exist for (`career-high-cards`/`career-high-strip`, `franchise-ranks`/`franchise-ranks-striped`). Rank never gets a bar or a position axis: the totals beside it are what make it readable. Stripes are translucent grey so one value works on either Canva ground. Measures every drawn string and refuses to save an overflow | `tests/test_taj_gibson_retirement.py` |
