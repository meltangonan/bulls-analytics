"""Render the top Bulls contested-jumper seasons as a table-bar hybrid."""
...
DEFAULT_DATA = _REPO / "docs/visuals/2026-09-19-contested-jumpers/data/top15.csv"
DEFAULT_OUTPUT = _REPO / "output/contested-jumpers/table.png"
PRIMARY_HEADSHOTS = Path("/Users/meltangonan/projects/bulls-analytics/cache/headshots")

# Geometry follows pull_up_points_bars.py: bar plus three support columns.
WIDTH = 2600
ROW_HEIGHT = 190
TOP_BOTTOM = 175
PORTRAIT_X = 150
...
BAR_HEIGHT = 104
VALUE_GAP = 22
OUTSIDE_BELOW = 100  # Shorter bars carry their value outside, in ink.
SUPPORT_X = (1640, 1980, 2380)
INK = house.BLACK
RED = house.RED
GREY = "#5F5B57"
GREEN = "#218347"
...
def _signed(value: float) -> str:
    """Percentage-point difference with a true minus sign; a rounded zero carries no sign."""
    if abs(value) < 0.05:
        return "0.0"
...
def render(data_path: Path, output_path: Path, *, final: bool = False) -> Path:
    required = {"rank", "player_id", "player_name", "season", "c_fgm", "c_fg_pct",
                "c_vs_league_pp", "u_fg_pct", "nba_rank"}
    frame = pd.read_csv(data_path)
    missing = required - set(frame.columns)
    if missing:
        raise ValueError(f"CSV is missing columns: {', '.join(sorted(missing))}")
    frame = frame.sort_values("rank").reset_index(drop=True)
    house.ensure_headshots(frame.player_id)
    ...
    scale = (BAR_RIGHT - BAR_LEFT) / float(frame.c_fgm.max())
    ...
    ax.text(BAR_LEFT, header_y, "CONTESTED MAKES", ha="left", va="center",
            fontsize=28, color=INK, fontproperties=bold, zorder=3)
    for x, label in zip(SUPPORT_X, ("NBA\nRANK", "CONTESTED\nFG%", "UNCONTESTED\nFG%")):
        ax.text(x, header_y + 8, label, ha="center", va="center", fontsize=25, linespacing=1.05,
    ...
    for i, row in frame.iterrows():
        ...
        rel = float(row.c_vs_league_pp)
        rel_color = GREY if -2 <= rel <= 2 else (GREEN if rel > 2 else DARK_RED)
        ax.text(SUPPORT_X[1], y + 16, f"{float(row.c_fg_pct):.1f}%", ha="center", va="center",
                fontsize=30, color=INK, fontproperties=bold, zorder=3)
        ax.text(SUPPORT_X[1], y - 30, f"({_signed(rel)} vs. LA)", ha="center", va="center",
                fontsize=23, color=rel_color, fontproperties=bold, zorder=3)
        ax.text(SUPPORT_X[2], y, f"{float(row.u_fg_pct):.1f}%", ha="center", va="center",
                fontsize=30, color=INK, fontproperties=bold, zorder=3)

        width = float(row.c_fgm) * scale
        ax.add_patch(FancyBboxPatch(
            (BAR_LEFT, y - BAR_HEIGHT / 2), width, BAR_HEIGHT,
            boxstyle="round,pad=0,rounding_size=8", linewidth=0, facecolor=RED, zorder=1,
        ))
        inside = row.c_fgm >= OUTSIDE_BELOW
        ax.text(BAR_LEFT + width + (-18 if inside else VALUE_GAP), y, f"{int(row.c_fgm)}",
                ha="right" if inside else "left", va="center", fontsize=29,
                color="white" if inside else INK, fontproperties=bold, zorder=3)
