"""Bulls regular-season floater leaders, 2015-16 through 2025-26.

Raw NBA snapshots and reconciliation stay with this post. Rendering reuses them.
Floaters are the NBA floating-shot family: Floating Jump shot plus the driving
and driving-bank variants. The two driving labels only exist from 2015-16, so
earlier seasons are audited for coverage but never ranked; their absence is
unavailable classification, not an absence of floaters.
"""
...
DATA = ROOT / 'docs/visuals/2026-09-05-floater-season-leaders/data'
# League-wide shot snapshots are not post-specific: the same endpoint and
# parameters return the same rows for any post needing that season's baseline.
# Reuse an existing snapshot rather than storing a second copy of it; new pulls
# are still written into this post's own directory.
LEAGUE_CACHES = [DATA / 'league_raw',
                 ROOT / 'docs/visuals/2026-09-05-layup-season-leaders/data/league_raw']


def league_snapshot(name: str) -> Path:
    for cache in LEAGUE_CACHES:
        if (cache / name).exists():
            return cache / name
    return LEAGUE_CACHES[0] / name


RANK_FROM = 2015


def is_floater(labels: pd.Series) -> pd.Series:
    return labels.fillna('').str.lower().str.contains('floating')
...
def fetch_league_season(season: str) -> pd.DataFrame:
    ...
    for team_id in NBA_TEAM_IDS:
        path = league_snapshot(f'{team_id}_{season}.csv.gz')
        if not path.exists():
...
def fetch_league_totals(season: str) -> pd.DataFrame:
    path = league_snapshot(f'teams_{season}.csv.gz')
    if not path.exists():
...
def merge_relative_fg(top: pd.DataFrame, baseline: pd.DataFrame) -> pd.DataFrame:
    ...
    if result.league_floater_fg_pct.isna().any():
        raise ValueError('Missing seasonal floater baseline')
    result['rFG_PCT'] = (result.FG_PCT - result.league_floater_fg_pct) * 100
    return result
...
def add_relative_fg() -> None:
    ...
    for season in top.SEASON.unique():
        ...
        included = is_floater(league.ACTION_TYPE)
        excluded_value = included & ~league.SHOT_TYPE.eq('2PT Field Goal')
        # A floating label on a three-pointer is a long heave, not a floater
        # (for example Josh Giddey's 46-foot winner on 2025-03-27).
        # Quarantine it from the floater baseline and retain the audit count.
        ...
        rows.append(dict(SEASON=season, excluded_non_two_point=int(excluded_value.sum()),
                         league_floater_fga=league_fga,
                         league_floater_fgm=league_fgm,
                         league_floater_fg_pct=league_fgm / league_fga))
    ...
    baseline.to_csv(DATA / 'league_floater_baselines.csv', index=False)
    print(result[['RANK','PLAYER_NAME','SEASON','FGM','FGA','FG_PCT',
                  'league_floater_fg_pct','rFG_PCT']].to_string(index=False), flush=True)
...
def summarize(shots: pd.DataFrame, players: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    labelled = is_floater(shots.ACTION_TYPE)
    two_point = shots.SHOT_TYPE.eq('2PT Field Goal')
    # Floating labels on three-pointers are long heaves; exclude them from the
    # count and keep them visible in the audit rather than silently dropping.
    ...
    partition = result[['PLAYER_ID','FGA','OTHER_FGA']].rename(columns={'FGA':'floater_fga'})
    audit = audit.merge(partition, on='PLAYER_ID', how='left', validate='one_to_one')
    audit['partition_delta'] = audit.floater_fga + audit.OTHER_FGA - audit.FGA
...
def prepare() -> None:
    ...
    for year in range(2000, 2026):
        ...
        labels['included'] = is_floater(labels.ACTION_TYPE)
        ...
        print(season, 'shots',len(shots),'floater makes',result.FGM.sum(),
    ...
    # The driving-floater labels only exist from RANK_FROM. Earlier seasons stay
    # in the audit as coverage evidence, but ranking them would compare a
    # one-label era against a three-label era.
    ranked = everything.loc[everything.SEASON.str.slice(0, 4).astype(int).ge(RANK_FROM)]
...
def render() -> None:
    ...
    top = pd.read_csv(DATA / 'top15.csv')
    baseline = pd.read_csv(DATA / 'league_floater_baselines.csv')
    ...
    out = ROOT / 'output/floater-season-leaders'
    out.mkdir(parents=True,exist_ok=True)
    fig.savefig(out / 'floater_season_leaders.png',dpi=300,transparent=True)
    plt.close(fig)
    copy = dict(title='Bulls floater scoring leaders',subtitle='Top 15 in floaters made in one season, since 2015–16',
                scope='2015–16 to 2025–26 regular season · Chicago games only',
                definition='NBA floating-shot labels, including driving and bank floaters. The driving labels begin in 2015–16.',
                relative_fg='rFG% = floater FG% minus that season’s NBA floater FG%, in percentage points.',
                shot_share='% OF FGA = floater attempts / all field-goal attempts in Bulls games.',
    ...
    print(out / 'floater_season_leaders.png')
