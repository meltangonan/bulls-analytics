...
def fetch_bulls_totals(season: str) -> pd.Series:
    ...
    if len(player) != 1:
        raise ValueError(
            f"expected one {PLAYER_NAME} row for {season}, found {len(player)}")
    # Read the counting stats, not TEAM_ABBREVIATION. Filtering by team_id gives
    # that club's numbers but labels the row with the player's END-OF-SEASON
    # team: Hinrich's 2015-16 row carries Chicago's 35 games and 118 attempts
    # under an "ATL" label. Trusting the label would reject correct data.
